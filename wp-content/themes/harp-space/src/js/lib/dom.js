 ...
 * @param {HTMLElement} el
 */
const hasClass = curry((className, el) => !el ? el : el.classList.contains(className))
 ...
 * @return {string}
 */
const getAttribute = (name, el) => (el && el.getAttribute) && el.getAttribute(name)
 ...
 * @private
 */
const _getData = (name, el) => getAttribute('data-' + name, el)
...
const removeClass = curry((className, els) =>
  map(
    (el) => { el && el.classList.remove(className); return el },
    els
  ))

const toggleClass = curry((className, els) =>
  map(
    (el) => { el && el.classList.toggle(className); return el },
    els
  ))
