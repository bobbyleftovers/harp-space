const colors = require('../../../node_modules/colors') // eslint-disable-line
const nodePath = require('path')
const gulp = require('gulp')
const glob = require('../../../node_modules/glob-all')
const concat = require('gulp-concat')
const wrap = require('../../../node_modules/gulp-wrapper')
  ...
  copy ({ type, path }, upload = false) {
    return new Promise((resolve, reject) => {
      ...
      if (!path.match(/\.[0-9a-z]+$/i)) {
        throw `Error copying path ${path}` // eslint-disable-line
      }

      let src = path.match(/(\/customers\/.*|.[^\/]*)$/) // eslint-disable-line
      if (src && src[1]) {
        src = src[1]
      } else {
        reject('Error copying path: ' + path) // eslint-disable-line
      }

      let base = path.match(/src\/(\D[^\/]*)/) // eslint-disable-line
      if (base && base[1]) {
        base = base[1]
      } else {
        reject('Error copying path: ' + path) // eslint-disable-line
